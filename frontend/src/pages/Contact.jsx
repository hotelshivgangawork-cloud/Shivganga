...
const handleChange = (e) => {
  ...
  if (e.target.name === 'message') {
    ...
    // Clear the error when message becomes valid (10+ characters)
...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (err.response?.data?.errors) {
      ...
      setError(backendErrors[0]?.message || "Validation failed");
    } else {
      setError(
        err.response?.data?.message || "Something went wrong. Please try again."
      );
...
/>
  ...
  value={formData.message}
  onChange={handleChange}
  className={`w-full bg-transparent border-b py-3 text-primary text-lg placeholder-gray-300 focus:outline-none transition resize-none ${
    messageLength < 10 && messageLength > 0 
      ? 'border-red-500 focus:border-red-500' 
      : 'border-[#D8D3C9] focus:border-[#C9A24D]'
  }`}
  placeholder="Tell us about your requirements... (minimum 10 characters)"
/>
{messageLength < 10 && messageLength > 0 && (
  <p className="text-red-500 text-xs mt-1">
    Please enter at least {10 - messageLength} more character{10 - messageLength !== 1 ? 's' : ''}
  </p>
)}
            </div>
...
>
  ...
  <Send size={14} />
        ...
        {success && (
  <p className="text-green-600 text-center text-sm font-medium mt-4">
    {success}
  </p>
)}

{error && (
  <p className="text-red-600 text-center text-sm font-medium mt-4">
    {error}
  </p>
)}
